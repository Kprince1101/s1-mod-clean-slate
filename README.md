...
write our own thin wrapper directly against the game's auto-generated `Il2CppScheduleOne.*`
...
`Il2CppScheduleOne.UI.NotificationsManager` singleton exists, then calls it through our own
wrapper (`CleanSlate/Game/Notifications.cs`) to show an in-game notification. This is deliberately
the first thing built: it proves build -> deploy -> MelonLoader load -> our own code reaching into
the game all work, before any real feature code gets written.
...
- `CleanSlate/` — the mod project.
  ...
  - `Game/` — our own wrapper layer around raw `Il2CppScheduleOne.*` types. Grows one file at a
    time, only for what a real feature actually needs.
